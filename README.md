...
온보딩에서 회원이 둘 중 하나를 선택합니다.

| 선택 | 발행 대상 | 대상 |
|---|---|---|
| `challenge` (기본) | 그 달의 **N월 블로그 챌린지 참여방** | 챌린지 참여 회원 |
| `sharing` | **포스팅 공유방** (17) | 챌린지 비참여 회원 |

### ⚠️ 저장하는 것은 게시판 "번호"가 아니라 "종류"입니다

이 카페는 달마다 새 챌린지 게시판을 만듭니다 (1월=2 → … → 7월=16 → 8월=19).

회원이 고른 순간의 menuid를 그대로 저장하면, **9월이 되어도 8월 게시판에 계속 발행됩니다.**
그래서 `members.board_target` 에는 `challenge` / `sharing` 만 저장하고,
실제 번호는 발행 직전에 `src/lib/boards.ts` 의 `resolveMenuId()` 가 환경 변수에서 읽습니다.

```
CAFE_MENU_ID_CHALLENGE=19   ← 달이 바뀌면 여기만 갱신
CAFE_MENU_ID_SHARING=17     ← 고정
```

달이 바뀌면 Vercel 환경 변수 **한 곳**만 바꾸면 챌린지 회원 전원에게 반영됩니다.
회원별로 번호를 저장했다면 매달 30명 전원을 손으로 갱신해야 합니다.

`members.target_menu_id` 는 **위 두 종류로 커버되지 않는 예외 회원용**입니다.
값이 있으면 `board_target` 보다 우선합니다. 보통은 비워 둡니다.

회원 온보딩은 **로그인 → 블로그 아이디 입력 → 게시판 선택**, 세 단계로 끝납니다.
